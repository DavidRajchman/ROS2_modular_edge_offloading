...
## 7. DiscoveryService (Network Service Discovery and Configuration)
...
The DiscoveryService maintains an in-memory registry of all active components with information including:
* ID (16-bit: group_id + instance_id)
* Component type (VHC, MEC, BRIDGE, etc.)
* Network location (IP address, port)
* Active status
* Last seen timestamp
* Configuration data

### 7.4 ID Structure and Management
* **ID Format:** 16-bit combined value where:
...
* Future versions will implement more sophisticated metrics for optimal assignment

### 7.7 What-if Scenarios and Solutions
* **VHC or MEC connects to DiscoveryService before Bridge:** DiscoveryService will send a "wait" message with proper error code ("No bridge connected"). It will periodically resend the same message to the MEC or VHC. As soon as a Bridge connects, it will send the proper configuration message.
...
* **Error Handling:**
  ...
  * Components failing to respond to keepalives are marked inactive and removed from routing decisions

### 7.9 Technical Implementation
* Implemented in C++ using the TransportLib library
* Uses a single-threaded event loop design with non-blocking I/O (select/poll)
* Lightweight text-based protocol with semicolon-delimited fields
* Human-readable messages for easy debugging and protocol inspection
...
* **Message Format:** Fixed header followed by semicolon-delimited fields
  ...
  * Missing/empty fields represented by consecutive semicolons
  * Human-readable message as the last field for logging/debugging
...
* **Message Types:**
  ...
  * Registration acknowledgment (ACK)
  * Error response (ERR)
  * Keepalive ping (PNG)
  * Keepalive response (PON)

* **Field Ordering:** Fixed for each message type, allowing simple parsing logic

* **Implementation Notes:**
  * Field separation: All fields are separated by semicolons (;)
