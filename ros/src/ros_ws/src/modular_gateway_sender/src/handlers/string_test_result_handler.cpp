...
void StringTestResultHandler::initialize()
{
  ...
  topic_name_ = node_->get_parameter("string_test_result_handler.topic").as_string();
  ...
  const MessageOptions& options)
{
  ...
  if (it == publishers_.end()) {
    ...
    logger_.Info("string_test_result_handler.cpp: Created publisher for topic: {}", topic);
  }
  ...
  if (options.serialized) {
    ...
    logger_.Info("string_test_result_handler.cpp: Published to topic {}: {}", 
              topic, msg.data);
  }
